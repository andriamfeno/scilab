...
frame_image = uicontrol(f, "style","frame", ...
            "position",[30, 100, 790, 530]); // en pixels
...
amelioration = uicontrol(f, ...
    ...
    "fontsize", 14, ...
    "foregroundcolor", [0 0 0], ...   // noir
    "backgroundcolor", [0.9 0.9 0.9], ... // fond gris clair
...
// 16. Créer un bouton Flou médian
...
// Fonction utilitaire : affiche une image dans le frame global "frame_image"
function showImageInFrame(img)
    // Récupérer la figure parente du frame global
    global frame_image;
    fa = get(frame_image, "parent");

    // Taille de la figure
    fig_size = get(fa, "figure_size"); // [largeur, hauteur]

    // Position du frame en pixels
    pos = get(frame_image, "position"); // [x, y, largeur, hauteur]

    // Créer des axes correspondant au frame
    ax = newaxes();
    ax.axes_bounds = [200,200,300,300];

    // Afficher l'image dans ces axes
    imshow(img);
    drawnow(); // forcer le rafraîchissement
endfunction

function importer_image()
    [filename, pathname] = uigetfile(["*.jpg";"*.png";"*."], "Choisir une image");
